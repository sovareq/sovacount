...
// Non-macOS stub: het launcher-crate bouwt op alle workspace-CI-platforms,
// maar de echte UI-shell vereist wry+tao+macOS system-libs (WKWebView,
// AppKit). Op Linux/Windows wordt een dummy-main gecompileerd die een
// duidelijke fout print en exit'et. Dit voorkomt dat workspace-CI faalt
// op `cargo build --workspace` op niet-macOS runners (zoals gebeurde met
// `glib-sys` op ubuntu-latest).
#[cfg(not(target_os = "macos"))]
fn main() {
    eprintln!(
        "[sovacount-launcher] this binary is macOS-only. Use the CLI \
         (`tier-classify`) or HTTP server (`governor-http`) directly on \
         this platform."
    );
    std::process::exit(2);
}
