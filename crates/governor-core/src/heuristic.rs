...
//!
//! Decision rules derived from `~/Sovareq/_workspace/conventions/token-governor-tags.md`:
...
///
/// The returned response leaves `model_hint` empty — the classifier fills it
/// in from `Config::tier_mapping` after the fast-path returns.
pub fn fast_path(req: &ClassifyRequest) -> Option<ClassifyResponse> {
    ...
    if big_loc || big_files || many_markers {
        return Some(ClassifyResponse {
            ...
            estimated_cost_usd: estimate_cost_usd(Tier::Op, OP_INPUT_TOKENS, OP_OUTPUT_TOKENS),
    ...
    if tiny_loc && single_file && no_ssot && no_markers {
        return Some(ClassifyResponse {
            ...
            estimated_cost_usd: estimate_cost_usd(Tier::Hk, HK_INPUT_TOKENS, HK_OUTPUT_TOKENS),
            alternative_tiers: vec![],
            from_cache: false,
        });
    }
...
mod tests {
    ...
    fn hk_fast_path_typo_fix() {
        ...
        let resp = fast_path(&r).expect("hk fast-path should fire");
    ...
    fn op_fast_path_big_loc() {
        ...
        let resp = fast_path(&r).expect("op fast-path on LOC");
    ...
    fn op_fast_path_many_files() {
        ...
        let resp = fast_path(&r).expect("op fast-path on files");
        assert_eq!(resp.tier, Tier::Op);
    }
    ...
    fn op_fast_path_two_markers() {
        ...
        let resp = fast_path(&r).expect("op fast-path on markers");
        assert_eq!(resp.tier, Tier::Op);
    }
